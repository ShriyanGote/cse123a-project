...
The sensor data is converted to grams earlier in the code, so with an already calculated value all the HTTP code needs to do is package it and send it as a POST request. The post data is sent using the built in ESP32 send\_http\_post(). This function is a part of the ESP32 utilities. For production, HTTP data would be sent using C sockets. 
