...
The cost of this custom board will depend on the available components and their individual prices. With the design we have created, the estimated cost for the components of 1 board is \$9.99. This price does not include getting a bare board from a fab, or the cost to place the components. As production scales these costs will go down slightly as higher quantities of materials get a discounted per unit price. This board is designed to be simple to manufacture so that time is not spent in the build process. 
...
Using a voltage divider to GPIO pin 2, a rough estimate of battery charge can be included in software. The board is set up to make this possible with the proper software implementation. By dividing down the voltage from the battery input, measurements can be made occasionally to determine the voltage output from the battery. This can be used with the knowledge that Lithium Ion batteries have a range in voltage output depending on their store of charge. The lower the output the lower the charge. 
